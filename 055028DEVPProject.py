...
data_path = r"C:\Users\OISHIK BANERJEE\Downloads\Oiii\Imports_Exports_Dataset.csv" 
...
sdset = dataset.sample(n=min(3001, len(dataset)), random_state=55028)  
...
sdset['Date'] = pd.to_datetime(sdset['Date'], format='%d-%m-%Y', errors='coerce') 
...
if selected_chart == "Payment Method Distribution":
    ...
    if 'Value' in filtered_data.columns and not filtered_data.empty:
        ...
        figa = go.Figure(go.Indicator(
            ...
            gauge={
                ...
                'bar': {'color': "darkblue"},
                'steps': [
                    {'range': [0, average_value * 0.5], 'color': "black"},
                    {'range': [average_value * 0.5, average_value], 'color': "black"},
                    {'range': [average_value, filtered_data['Value'].max()], 'color': "black"}
                ],
                'threshold': {
    ...
    plt.title('Average Total Value for Top 5 Products', fontsize=16)
